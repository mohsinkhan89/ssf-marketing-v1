@php
    $editorDescription = old('description', $blogRecord?->description ?? '<p></p>');
...
<form class="dashboard-form" method="POST" action="{{ $action }}" enctype="multipart/form-data" data-rich-text-form>
    @csrf
    @if ($method)
        @method($method)
    @endif
    <label><span>Title</span><input type="text" name="title" value="{{ old('title', $blogRecord?->title) }}" placeholder="The 2025 SEO Playbook" required data-slug-source></label>
    <label><span>Slug</span><input type="text" name="slug_preview" value="{{ old('slug_preview', $blogRecord?->slug) }}" placeholder="the-2025-seo-playbook" readonly data-slug-target></label>
    <label class="full-span"><span>Short description</span><textarea name="short_description" rows="3" placeholder="Short summary shown on blog cards and detail intro">{{ old('short_description', $blogRecord?->short_description) }}</textarea></label>
    <label><span>Card image</span><input type="file" name="card_img" accept="image/*"></label>
    <label><span>Banner image</span><input type="file" name="banner_img" accept="image/*"></label>
    @if ($blogRecord?->card_img || $blogRecord?->banner_img)
        <div class="full-span blog-image-preview-row">
            @if ($blogRecord?->card_img)<span><img src="{{ url($blogRecord->card_img) }}" alt="Card image preview">Card image</span>@endif
            @if ($blogRecord?->banner_img)<span><img src="{{ url($blogRecord->banner_img) }}" alt="Banner image preview">Banner image</span>@endif
        </div>
    @endif
    <label class="full-span rich-text-field">
        ...
        <div class="rich-text-toolbar" aria-label="Description formatting tools">
            ...
            <select data-rich-command="formatBlock" title="Text format"><option value="P">Paragraph</option><option value="H1">Heading 1</option><option value="H2">Heading 2</option><option value="H3">Heading 3</option><option value="H4">Heading 4</option><option value="BLOCKQUOTE">Quote</option><option value="PRE">Code block</option></select>
            <select data-rich-command="fontSize" title="Font size"><option value="3">Normal</option><option value="2">Small</option><option value="4">Large</option><option value="5">Extra large</option><option value="6">Huge</option></select>
    ...
    <label><span>Status</span><select name="status" required><option value="published" @selected(old('status', $blogRecord?->status ?? 'published') === 'published')>Published</option><option value="draft" @selected(old('status', $blogRecord?->status ?? 'published') === 'draft')>Draft</option></select></label>
    <button class="primary-action form-submit" type="submit">{{ $method ? 'Save blog' : 'Add blog' }} <i class="fa-solid fa-arrow-right"></i></button>
</form>